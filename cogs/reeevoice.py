...
class Music(commands.Cog):
    ...
    async def do_vote(self, ctx, player, command: str):
        ...
        if ctx.author.id in attr:
            ...
            await ctx.send(f"**{ctx.author}** has voted to {command} the song."
    ...
    async def search_tracks(self, ctx, *, query: str):
        ...
        fmt = [f"**{index}.** {track} [{player.format_delta(track.duration)}]" for index, track in
               enumerate(tracks, 1)]
    ...
    async def queue_(self, ctx):
        ...
        fmt = "\n".join(
            f"**{index}. `{str(song)}` [{player.format_delta(song.duration)}]**" for index, song in
            enumerate(upcoming, 1)
        )
